...
// 监测链接设置
class ProjectUpdatePostParamTrackUrlSettingData extends Data
{

    /**
     * @var string $track_url_type 监测链接类型，区分使用监测链接组或者自定义链接允许值：CUSTOM自定义链接（默认值）、GROUP_ID监测链接组landing_type为LINK、landing_type=DPA且dpa_adtype=DPA_LINK、MICRO_GAME、QUICK_APP、SHOP时，不支持传入，传入报错
     */
    public string $track_url_type;
    
    /**
     * @var int $track_url_group_id 监测链接组id，当 track_url_type =GROUP_ID时有效且必填通过【事件资产下创建监测链接组】获得
     */
